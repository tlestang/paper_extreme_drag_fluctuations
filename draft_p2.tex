...
For $k \gtrapprox k_{max}$ the majority of the clones are resampled, with the expection of only one or two.
Because the resampling is based on a very small subset of the ensemble of $N_c$ trajectories, most of the trajectories in the biased ensemble overlap.
This effect is visible in figure~\ref{fig:illustration_IS}, where the histogram corresponding to the largest value of $k$ is peaked around a single value.
...
\begin{table}
  ...
  \begin{tabular}{l|ccccc}
    ...
    $\mathcal{N}_{direct}$ & 2601.258& 374.862& 24.268& 0.958& 0.000\\ 
  \end{tabular}
...
\begin{figure}
  ...
  \caption{Importance sampling for the time-averaged drag acting on the obstacle embedded in the test flow. The drag is averaged over $T=10\tau_c$. 
  The \ac{gktl} algorithm is used with $N_c = 16384$ copies and $T_a = T= 10\tau_c$ for three increasing values of the bias $k$. Additionally, the cloning period is $\tau = \tau_c/2$, resulting in $M=20$ iterations of the evolution/selection procedure.}
\end{figure}
