...
\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{D2Q9}
...
\begin{figure}
  \centering
 % \includegraphics[width=.6\linewidth]{return_time_drag}
...
In addition, figure~\ref{fig:contribution_forebody_and_base_pressure} hints that, the larger the fluctuation, the more important is the contribution of the base pressure fluctuation, relatively to the forebody pressure fluctuation.
...
We now turn to a description of the flow dynamics for some of the extreme drag fluctuations, sampled in the control timeseries.
Figure~\ref{fig:top_four_events} displays the vorticity field at $t=t^{\star}$ for the four events corresponding to the largest fluctuation amplitudes.
One can see that they correspond to very similar flow configurations.
In each case, a strong vorticity region is located in the vicinity of the base of the obstacle, where vorticity levels typically reach twice as much as the typical vorticity fluctuations displayed in figure~\ref{fig:typical_event}.
Such strong vorticity is produced through viscous shear along either the bottom or top boundary of the obstacle.
...
As illustrated by figure~\ref{fig:top_four_events_streamlines}, vorticity generated alongside the boundary has no other choice but to concentrate close to the base of the obstacle.

Figure~\ref{fig:top_four_events_timeseries} displays the drag timeseries corresponding to the four events illustrated in figure~\ref{fig:top_four_events}.
It illustrates that the lifetime of such fluctuations temporal extension is roughly one correlation time unit $\tau_c$.
It is the time it takes for the block eddy to form and be advected away by the mean flow, thus freeing the vorticity concentrated in the vicinity of the base of the obstacle.
This can be seen in figure~\ref{fig:dynamics}, which illustrate the vorticty dynamics from $t^{\star}-\tau_c/2$ to $t^{\star}$ on a particular example of an extreme drag fluctuation. 
On can see in figures~\ref{fig:dynamics_a} to~\ref{fig:dynamics_g} that, from roughly $t^{\star}-\tau_c/2$ to $t^{\star}-\tau_c/3$, the top boundary layer is perturbed by an incoming eddy.
This interaction eventually leads to the formation of a region of negative vorticity at roughly one square length of the base of the obstacle.
This can be seen see in figure~\ref{fig:dynamics_k}.
Although this vorticity is too far from the base to directly affect the base pressure, and thus the drag, it constrains the positive vorticity generated alongside the bottom boundary of the obstacle---see figures~\ref{fig:dynamics_k} to~\ref{fig:dynamics_p}.
...
Figure~\ref{fig:gamma_asof_fd} shows $\overline{\gamma}$ as a function of the instantaneous drag $f_d$, for $t^{\star}-2\tau_c \leq t \leq  t^{\star}+2\tau_c$, for each of the sampled events that follow the previously described dynamics..
...
This hypothesis is motivated by figure~\ref{fig:pdf_averared} (\textbf{figure manquante}).
