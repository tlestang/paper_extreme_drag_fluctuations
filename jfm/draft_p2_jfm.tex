...
More importantly, these types of algorithms have been shown to be useful for the study of rare events in simple deterministic dynamics~\citep{wouters2016rare}.
%
%
\EL{An original contribution of our study is certainly to test the application of rare-event sampling algorithms in the context of far-from-equilibrium dynamics with an irreducible very large number of degrees of freedom. 
%
...
It is found in particular that sampled extreme events for the instantaneous drag result from very similar dynamics. %Furthermore, extreme fluctuations for the time-averaged drag can be connected to the statistics of the instantaneous drag.
...
%
\EL{Rare-event (sampling) algorithms compute an ensemble of $N$ trajectories $\{\mathbf{x}_n(t)\}_{0\leq t \leq T_a}$ with $n=1 \cdots N$, where $\{\mathbf{x}_n(t)\}_{0\leq t \leq T_a}$ refers to a trajectory of duration $T_a$ in the phase space of the system.
At each step of the algorithm, certain trajectories are discarded and others are replicated in order to preferably sample trajectories with extreme fluctuations. The algorithm tracks the ratio of the probability of the new ensemble with the probability of the previous one, which makes it possible to estimate the statistical bias and, therefore, to infer the statistics of extreme events. 

Different algorithms use different \emph{selection rules}. 
The success of the algorithm essentially depends on the quality of the selection rule to select the precursors of the extreme event \citep{rolland_statistical_2015}. 
...
\EL{In the version of the \ac{ams} algorithm used in this work,  %a sequence of events that can be sampled with a higher probability  
trajectories have a fixed duration $T_a$~\citep{lestang_computing_2018}.
...
%
\EL{In brief, trajectories are first evolved in time  independently up to $t=\tau $ with $\tau < T_a$ referring to a cloning period. Then, the selection-and-cloning rules apply according to the average of the observable of interest, here $f_d(t)$, over the interval $[0,\tau]$. 
This procedure is repeated $n-1$ times over the intervals $[\tau, 2\tau],~...,~[(n-1)\tau, n\tau = T_a]$. As a result, the resampled trajectories are distributed according to a probability distribution that is tilted towards large values of the averaged observable. Further details are provided in the Appendix \ref{app:gktl_description}. }
...
As for the \ac{ams} algorithm, $N$ governs the error affecting the averaged quantities evaluated from the biased ensemble of trajectories, and should be chosen as high as possible. Finally, the cloning period $\tau$ determines how often the resampling is performed. The choice of the cloning period $\tau$ is important in practice.
A  cloning period too-short can result in a loss of information if the clones do not separate from their parents between two cloning steps. On the contrary, choosing $\tau$ much higher than  $\tau_c$  results in insufficient cloning steps to allow for efficient importance sampling.
...
%%
\EL{Fig.~\ref{fig:IS_GKTL} shows the histograms of the biased \ac{pdf}s of the (zero-mean) time-averaged drag for the three ensembles of trajectories, in addition to the unbiased \ac{pdf} based on the control run (with the same computational cost). 
...
This effect is highlighted in Fig.~\ref{fig:IS_GKTL} where the biased \ac{pdf} corresponding to $k=0.03$ becomes artificially peaked.
%
...
\EL{In the present simulations with $N=1024$ trajectories, one can empirically estimate that  $k$ should be smaller than $k_{max} \approx 0.03$ to ensure the independence of the trajectories in the biased ensemble.}
...
One observes that extreme events are consistent with the picture of strong vorticity being trapped in the vicinity of the base of the obstacle, as pointed out in section~\ref{sec:instantaneous_drag}.
Nevertheless, it should be stressed that the computational cost for sampling the events shown in Fig.~\ref{fig:timeseries_extrms_AVG_GKTL} and Fig.~\ref{fig:illustr_extrms_vorticity_GKTL} is roughly one hundred time lower than the computational cost required to capture by direct sampling the events displayed in  Fig.~\ref{fig:top_4_events_vorticity} and Fig.~\ref{fig:extreme_avg}.
...
%While such algorithms have already proved to be efficient for a wide range of dynamical systems, its applicability in the context of turbulent flows remains open.   
...
%For complex dynamics such as turbulent flows, this choice is difficult because of the lack of characterisation of rare events. 
%These latter are expected to depend on the dynamical regime, the stirring mechanism and boundary conditions of the flow. 
