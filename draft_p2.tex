...
The main idea is here to replace a long simulation by a set of much shorter ones, running in parallel, with dynamics that are replicated or pruned in order to sample large-amplitude events more frequently. 
%
Such techniques have been shown to be efficient for a wide range of problems in statistical physics, computer science, biochemistry, enabling the simulation of rare  events out of reach by brute-force direct sampling.
Application to fluid-structure interaction problems remains mainly open. 
%
The drag experienced by a squared obstacle placed in a turbulent flow (in two dimensions) is here taken as a representative case study to investigate the performance of two major rare-event sampling algorithms, namely the \ac{ams} and the \ac{gktl} algorithms.
...
%
In this paper, a more general approach is considered for dynamical systems for which an action cannot be easily derived or minimized. 
...
%
Fig.~\ref{fig:typical_vorticity} displays the typical evolution of the vorticity field around the obstacle over a few turnover times. It is observed that, typically, some vorticity  is generated near the front part of the obstacle, rapidly detaches from it, and is advected downstream by the flow.
Therefore, the \textit{base pressure} varies much less than the \textit{forebody pressure} and typical drag fluctuations mostly result from pressure variations at the front of the obstacle.
In section~\ref{sec:direct_sampling}, we shall highlight that the situation leading to  {extreme} fluctuations of the drag is very different.
...
\begin{figure}
  ...
  \textbf{(b)} Autocorrelation function of the drag defined as $C(\tau) = \overline{ \tilde f_d(t+\tau)\tilde f_d(t)} ~/~ \overline{{\tilde f_d}^2}$. The correlation time $\tau_c\simeq 4 \tau_0$ is obtained by the condition $C(\tau_c)=0$.
...
\begin{figure}
	...
	\caption{Amplitude of drag fluctuation (in standard-deviation unit) as a function the corresponding return time.
	}
...
How rare is a fluctuation $f_d \geq a$ can be quantified by its \textit{return time} $r(a)$. This latter is defined as the averaged time between two consecutive occurrences of peak fluctuations with amplitude $f_d \geq a$, as illustrated in Fig.~\ref{fig:illustrate_return_time}.


Extreme drag fluctuations are \textit{rare events} in the sense that the return time is much larger than the correlation time, \emph{i.e.} $r(a) \gg \tau_c$.
...
%
Fig.~\ref{fig:return_time_instant} shows the evolution of the return time $r(a)$ with the amplitude of fluctuation $a$ computed from {direct sampling} of the drag signal $f_d(t)$ \cite{lestang_computing_2018}. It is consistently found that the return time  diverges  exponentially with the amplitude of fluctuation. Let us also remark that under-sampling effects explain some deviation from the exponential law at the largest amplitudes.
...
\end{equation}
where $\tilde{p}_{fb}^{\star}$ and $\tilde{p}_{base}^{\star}$ denote the variations of the forebody and base pressure, respectively.
%
...
\end{equation}
where $f_d(t)$ denotes the instantaneous drag and $T$ is the investigated timescale (response time). We shall consider $T=10\tau_c$ in the following.
...
%
In the limit of very rare events and complex dynamics such as turbulent flows in industrial or environmental domains, the computational cost of  direct sampling becomes prohibitive. According to Eq.~\eqref{eq:return_time}, the return time of fluctuations $f_d \geq a$ scales as $r(a) \propto e^{\lambda a}$.
...
{A very recent review of the \ac{ams} algorithm, its history and applications are provided in~\cite{cerou2019adaptive}.}
...
The computational cost is many orders of magnitude lower than that obtained by direct sampling.
...
In practice, a small perturbation in the state of the flow has been introduced at the branching points (see Fig.~\ref{fig:illustr_AMS}). 
%
...
We assume that this perturbation at branching times has a negligible impact on the properties of the sampled rare events.
...
It takes a certain time $\tau_L$ (called Liapunov's timescale) before a re-sampled trajectory separates from its parent. In our situation, this ``memory effect'' originates from the deterministic nature of fluid dynamics. 
%
...
On the other hand, finding a better score function would require an intricate understanding of the flow scenario leading to extreme events.
...
A key feature of the \ac{gktl} algorithm is that the sampling procedure do not involve any alteration of the dynamics. All trajectories in the resampled ensemble are solutions of the original dynamical. 
...
We have taken $N=16384$ and $T_a = 10\tau_c$ which yields $C_{gktl} \approx 1.6 \times 10^5 \tau_c$. The choice of the cloning period $\tau$ is an issue.
...
It compares return times obtained for two values of the bias $k$ and a fixed computational cost $N \times T_a$ to the return times obtained by a direct approach applied to a single timeseries of duration corresponding to $T_{tot} = N \times T_a$.
...
\begin{figure}
	...
	\caption{\label{fig:return_times_gktl} Return times for the time-averaged drag acting on the square obstacle. $\tilde{F}_T$ denotes the time-averaged drag with zero mean.
		The blue and read lines are obtained from the biased ensemble of trajectories generated by the \ac{gktl} algorithm with $N=1024$ and $T_a=30\tau_c$. The green line is the return times obtained from a single timeseries of duration $T_{tot} = N\times T_a$. Uncertainty ranges are shown.}
\end{figure}
...
The efficiency of the algorithm depends on the phenomenology of the dynamics and, therefore, on the choice of the score function.
