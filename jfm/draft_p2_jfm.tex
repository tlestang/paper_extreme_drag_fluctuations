...
%
Two algorithms a priori suitable for such dynamics are considered, namely, the Adaptive Multilevel Splitting (AMS) algorithm \citep{cerou_adaptive_2007} and the Giardina-Kurchan-Tailleur-Lecomte (GKTL) algorithm \citep{giardina_direct_2006}.}
%
...
$\Delta p_{fb}^{\star}/\tilde{f}_d^{\star}$ and $-\Delta p_{base}^{\star}/\tilde{f}_d^{\star}$ to \EL{the drag fluctuation $\tilde f_d^{\star}$.
	It is found that very large fluctuations of the drag result mainly ($\sim 80\%$) from the drop of the base pressure, whereas the variation of forebody pressure contributes much less to $\tilde{f}_d^{\star}$. On the contrary, moderate fluctuations arise from combined variations of the forebody and base pressures without any particular predominance, which is in agreement with the previous observations (see Fig.\ref{fig:typical_vorticity}).}
...
\EL{In the following, we shall consider $T=10\tau_c$, where $\tau_c$ is the correlation of the instantaneous drag.
...
%
In the limit of very rare events and complex dynamics such as turbulent flows, the computational cost of  direct sampling becomes prohibitive. Indeed, the return time of fluctuations $f_d \geq a$ scales as $r(a) \propto e^{l a}~(l>0)$ according to Eq.~\eqref{eq:return_time}.
As a consequence, the computational cost required to sample events of amplitude $f_d \geq a$ through brute-force sampling diverges typically as $e^{l a}$.
\textbf{The direct sampling approach used in the previous sections to sample extremes of the drag is therefore not scalable and it would be difficult, if even feasible, to study rarer events or more complex flows.}
...
\sout{The aim of rare-event sampling algorithms is to sample extreme fluctuations for a much lower computational cost.}
\textbf{In this section, we investigate the use of rare-events algorithms in order to sample extreme events for a
...
\ZZ{Rare event algorithms compute an ensemble of $N$ trajectories $\{\mathbf{x}_n(t)\}_{0\leq t \leq T_a}$ with $n=1 \cdots N$, \textbf{where $\{\mathbf{x}_n(t)\}_{0\leq t \leq T_a}$ refers to a trajectory of duration $T_a$ in the phase space of the
system.}
Then at each time step of the algorithm, some trajectories are discarded and others are replicated, in order to make trajectories with largest value of the extreme more common. The algorithm tracks the value of the ratio of the probability of the new ensemble with the probability of the previous one, in order to be able to estimate the probability of the extreme event in the model. Different algorithms use different selection rules. The success of the algorithm depends on the quality of the selection rule in order to select the precursors of the extreme event. Optimal selection rules are called committor functions, see for instance~\citep{lestang_computing_2018} and references therein. In the following we consider the \acl{ams} (\ac{ams}) and \acl{gktl} (\ac{gktl}) algorithms. They both proved efficient for various dynamics and adopt opposite strategies:  the \acl{ams} algorithm uses as a selection rule the value of a predetermined score function, while the \acl{gktl} uses as a selection rule based on the increment of a score function.} \ZZ{A complete description of these two algorithms and their operating details is given in appendixes.}
...
% The first use of the \ac{ams} algorithms for more complex dynamics, for instance stochastic partial differential equations was for the computation of rare trajectories in the Allen-Cahn equations \citep{rolland_computing_2016}. 
 More recently \EL{it} has been applied to rare events in stochastic models of wall-turbulence \citep{rolland_extremely_2018} and atmospheric dynamics \citep{bouchet2019rare}. \ZZ{A review of the \ac{ams} algorithm, its history and applications is also available in \citep{cerou2019adaptive}}. 
%

During the last decade, the main theoretical framework for the study of rare events in statistical physics has been the theory of large deviations \citep{touchette_large_2009}.
Alongside numerical methods have been developed to sample rare events \citep{DelMoralBook}. 
Among them the \ac{gktl} algorithm \citep{giardina_direct_2006} is particularly suited for \ZZ{estimating the probability of observables which are of time integrals over a very long time \citep{giardina_simulating_2011,Laffargue_2013}.
%
Recently the \ac{gktl} algorithm has proven extremely efficient to simulate extreme heat waves in a comprehensive climate model \citep{ragone_computation_2018}, with a gain of a factor 100 to 1000 for the computation time.}
This achievement represents a significant leap in the applicability of rare-event sampling to complex dynamical systems. 

\ZZ{The aim of this part of the work is to test rare-event algorithms for fluid-structure interaction in a turbulent flow, which a completely new field of application.} 
...
% introduce score function and optimal score function
\textbf{In the \ac{ams} algorithm, trajectories are iteratively resampled in order to maximise the maximum value of a score function $\xi(\mathbf{x})$ along the trajectories.
...
%
 At each iteration the trajectories with the lowest maxima of the score function $\xi (\mathbf{x}(t),t)$ during the time interval $0\leq t \leq T_a$ are discarded. 
These trajectories are re-sampled by branching them from the remaining trajectories.
\EL{The operating principle of the \ac{ams} algorithm is detailed and illustrated in Appendix \ref{app:ams-short}.}


The \ac{ams} algorithm is here used to sample flow evolutions that exhibit extreme fluctuations of the drag $f_d(t)$ acting on the square obstacle.

% Discuss naive choice for the score function, in line
% with the objectives of the paper
\textbf{The score function is set as the drag itself, that is $\xi(\mathbf{x}) = f_d(\mathbf{x})$.
...
\ZZ{In the version of the \ac{ams} algorithm we use in this work, we consider %a sequence of events that can be sampled with a higher probability  
...
%
Let us also stress that the Navier--Stokes dynamics is deterministic.
\ZZ{Then, small random perturbation are artificially introduced at branching points. This procedure is detailed in Appendix \ref{app:perturb_branching_time}}.
...
% (see Fig.~\ref{fig:illustr_AMS} and~\cite{wouters2016rare}).
	%
Due to the chaotic dynamics, re-sampled trajectories separate rapidly from their parents, on a timescale of the order of the Lyapunov \textbf{not introduced} time.}
...
\ZZ{Based on linear response theory, we expect this small perturbation to have a negligible impact on the statistical properties of the sampled rare events.} This has been tested by performing a long simulation of the dynamics and by regularly perturbing the flow.
It was checked that the obtained statistics of the drag were consistent, within an error of the same amplitude as the perturbation amplitude, with the statistics computed from the (unperturbed) control simulation. %\EL{Freddy: provide reference for Lyapunov timescale. \#12}
...
\EL{In addition to the score function, two important parameters are the number $N$ and the duration $T_a$ of the trajectories. The size of the ensemble $N$ governs the statistical error affecting quantities averaged over the sampled set of trajectories. Therefore, $N$ should be taken as large as possible to reduce these errors with a practical limit given by the available computational resources. In this work, we have performed two numerical experiments with $N=32$ and $N=256$.}
\ZZ{The value of $T_a$ should be chosen larger than $\tau_c$~\citep{lestang_computing_2018}, but kept small enough in order to minimize the computational cost.
  The duration of the trajectories was set to $T_a=5\tau_c$ in both cases.
  We checked that larger values of $T_a$ do not affect the result.}
...
\begin{figure}
	...
	\caption{\label{fig:AMS_drag_resampling} Maxima of the instantaneous drag throughout $256$ re-sampled trajectories (vertical axis) as a function of the corresponding \EL{computational cost $C_{AMS}$} in correlation-time unit. The \ac{ams} algorithm fails to efficiently sample rare trajectories associated to drag fluctuations higher than the largest fluctuation already captured in the initial ensemble. \QQ{Changed TAMS to AMS in horizontal axis}}
\end{figure}


%pitfall
%
\ZZ{Fig.~\ref{fig:AMS_drag_trajectories} displays the ensemble of drag values after 181 iterations of the \ac{ams} algorithm.} 
%
\textbf{It illustrates that trajectories in the final ensemble overlap over most of their duration and are replicates of the trajectory presenting the largest maximum in the initial ensemble.}
%
This result is also confirmed by increasing the number of initial trajectories to $N=256$, see figure~\ref{fig:AMS_drag_resampling}.
(\textbf{Leger remaniement ici})
Fig.~\ref{fig:AMS_drag_resampling} displays the maximum drag achieved by the re-sampled trajectories as a function of the computational cost.
Although the trajectories with the lowest maxima of the score function are discarded after a few iterations and new trajectories with higher maxima are re-sampled, the re-sampled trajectories never exceed the amplitude of the highest maximum already attained in the initial set of trajectories.
The algorithm therefore fails to enhance the sampling of extreme events.
...
%
\EL{As shown in section \ref{sec:dynamical_aspects}, extreme drag fluctuations have a lifetime $\tau_c$ related to the timescale over which a vortex remains trapped against the base of the obstacle. After $\tau_c$, the vortex is freed and carried away by the flow, and further large fluctuations of the drag can only result from the trapping of other vortices.
%
Fig.~\ref{fig:AMS_drag_trajectories} indicates that $\tau_c$ is shorter than the Lyapunov's timescale $\tau_L$. }
%
%
...
%
\textbf{Better results may be obtained by using a different score function, but at this stage we do not know how to build such a score function for this specific problem.}
...
%
\sout{At regular time intervals, some elements of the ensemble are killed and others are cloned according to a weight that depends on the history of the element itself.
%
...
\textbf{\EL{In brief, trajectories are first evolved independently up to time  $\tau < T_a$ with $\tau$ referring to a cloning period. Then, the selection-and-cloning rules apply according to the average of the observable of interest ($f_d(t)$) over the interval $[0,\tau]$ (see Appendix \ref{app:gktl_description} for details). 
This procedure is repeated $n-1$ times over the intervals $[\tau, 2\tau],~...,~[(n-1)\tau, n\tau = T_a]$. The resampled trajectories are distributed according to a probability distribution that is tilted towards large values of the averaged observable.}}
%
\EL{The operating principle of the \ac{gktl} algorithm is developed in Appendix \ref{app:gktl_description}.}

(\textbf{Is the following necessary? Could go in appendix. We already introduced provious applications of GKTL in the introduction of this section.})
\textit{The \ac{gktl} algorithm belongs to a family of algorithms called ``go with the winners'' \citep{aldous1994go,grassberger2002go}.
...
The application of a go-with-the-winners approach to the computation of large deviations in non-equilibrium systems has first been proposed in \cite{giardina_direct_2006}.
...
The application of the \ac{gktl} algorithm is considered for the flow dynamics introduced in section~\ref{sec:test_flow}.
\EL{The purpose is to speed-up the sampling of trajectories with extreme fluctuations of the \textit{time-averaged} drag, $F_T$.} 
The observable of interest is therefore the drag $f_d(t)$ and the duration $T_a$ of each trajectory corresponds to the period of averaging of the drag, \emph{i.e.} $T_a=T$.

\textbf{(The algorithm is already described above. I replaced the above (striked-out) by the following.)}
\textit{\EL{In brief, trajectories are first evolved independently up to time  $\tau < T_a$ with $\tau$ referring to a cloning period. Then, the selection-and-cloning rules apply according to the average of the observable of interest ($f_d(t)$) over the interval $[0,\tau]$ (see Appendix \ref{app:gktl_description} for details). 
This procedure is repeated $n-1$ times over the intervals $[\tau, 2\tau],~...,~[(n-1)\tau, n\tau = T_a]$. The resampled trajectories are distributed according to a probability distribution that is tilted towards large values of the averaged observable. }}

%
% Choice of parameters and perturbation

\EL{The algorithm depends on three parameters: the cloning strength $k$, the number of trajectories $N$ and the resampling period $\tau$. The higher $k$, the larger is the bias involved in the statistical resampling, as illustrated in Fig.~\ref{fig:IS_GKTL}.}
\textbf{There is however no general relation between this typical value and the value of $k$, and this value must be set empirically.}
\EL{As for the \ac{ams} algorithm, $N$ governs the error affecting the averaged quantities evaluated from the biased ensemble of trajectories, and should be chosen as high as possible. Finally, the cloning period $\tau$ determines how often the resampling is performed. The choice of the cloning period $\tau$ is important in practice.
\textbf{Too short a cloning period} \sout{A cloning period too-short} can result in a loss of information if the clones do not separate from their parents between two cloning steps. On the contrary, choosing $\tau \gg \tau_c$ may result in insufficient cloning steps to allow for efficient importance sampling.
As a result, a safe rule of thumb is to take $\tau$ of the order of $\tau_c$. \sout{In the following experiments, $\tau = \tau_c /2$.} (\textbf{Written below)}}

\QQ{Thibault, pour la suite, nous sommes restés sur une version proche du papier initial, par simplicité. Si tu penses que certains points que tu avais ajoutés sont vraiment nécessaire pour répondre au referee, dis le nous.}

(\textbf{Il faut mieux discuter le choix des parametres de l'algorithme}
)
\textbf{In practice, we started by fixing the computational cost associated with a single run of the cloning algorithm, \textit{i.e.} by fixing $N \times T_a$.
...
\sout{The computational cost $C_{{gktl}}$ depends on both the duration of each trajectory and the number of trajectories: $C_{gktl} = N \times T_a$.
We have taken $N=16384$ and $T_a = 10\tau_c$ which yields $C_{gktl} \approx 1.6 \times 10^5 \tau_c$.} (\textbf{Out-of-date values})
...
\ZZ{Fig.~\ref{fig:timeseries_extrms_AVG_GKTL} displays the drag signal for extreme trajectories resampled by the \ac{gktl} algorithm.}

\begin{figure}
	\centering
	\includegraphics[width=\linewidth]{timeseries_extrms_AVG_GKTL/timeseries_extrms_AVG_GKTL}
	\caption{Drag timeseries corresponding to the highest fluctuations of the average drag in the ensemble of resampled trajectories. The algorithm was applied with $N = 1024$, $\tau = \tau_c / 2$ and $k = 0.03$. }
	\label{fig:timeseries_extrms_AVG_GKTL}
\end{figure}
In addition, Fig.~\ref{fig:illustr_extrms_vorticity_GKTL} displays the vorticity field at the time of maximum drag over a subset of trajectories
...
\section{The \ac{ams} algorithm -- version initiale}
\label{app:ams-short}
...
%\end{figure}
%
In practice, we chose $\epsilon = 0.002$ and $N_s = 10$ to ensure that the perturbations remain sufficiently small, random and independent.  \QQ{is it correct?}.
...
%
In order to check that this perturbation does not impact the statistics of drag fluctuations, a long simulation of duration $T_{tot} = 10^5 \tau_c$ have been performed with a periodic perturbation (with period $\tau_c / 2$) mimicking the perturbation of the clones in the algorithm (see section \ref{sec:rare_events_algorithms}).
%
...
\caption{\EL{\ac{pdf}s of (normalised) drag fluctuations obtained from the perturbed (at branching points) and unperturbed numerical simulations; $\epsilon$ is the relative amplitude of the perturbation.} \QQ{indicate $x=(f_d-\mu)/\sigma$ and ${\cal P}(x)$  as before}}
