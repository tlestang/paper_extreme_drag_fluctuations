...
\begin{abstract}
\TL{  This study evaluates the relevance of rare-event sampling type approaches to make accessible the numerical simulation of extreme mechanical efforts exerted by a turbulent flow impinging onto a bluff body.
  The main idea is here to replace a long simulation by a set of much shorter simulations, running in parallel, which corresponding dynamics are replicated or pruned in order to sample large-amplitude events more frequently. 
%
  Such techniques have been shown to be efficient for a wide range of problems in statistical physics, computer science, biochemistry, enabling the simulation of events far from reach from brute force sampling approaches.
  Application to fluid-structure interaction problems remains mainly open. 
%
  The drag experienced by a squared obstacle placed in a turbulent flow (in two dimensions) is taken as a representative case study to investigate the performance of two major sampling algorithms, namely the \ac{ams} and the \ac{gktl} algorithms.
  Practical evidence is given that the fast sweeping time of fluid structures has a drastic influence on the efficiency of the sampling with both algorithms.
  While is is shown that the \ac{ams} re-sampling does not lead to rare trajectories with significant run-time savings, we illustrate that the \ac{gktl} algorithm allows for the simulation of extreme fluctuations of the time-averaged drag, as well as accurate computation of the associated statistics, for instance return times.}
\end{abstract}
...
Indeed, such anomalous constraints can threaten the structural integrity of embedded structures.
%
...
Nevertheless, this requires very long simulations since these events are also very rare.
Our motivation is to evaluate the practicality and efficiency of rare-event sampling techniques to alleviate this prohibitive computational cost. 
%	
	
% general def on rare-event sampling %
%
\TL{Rare-event sampling refer to a large body of methods that aim at sampling preferentially regions of phase space that correspond to rare events, that would otherwise be accessed with a very low probability through a brute-force sampling.}
%
...
%
An alternative approach is based on \emph{instantons} \cite{grafke2015instanton}. For stochastically driven systems in the limit of weak noise, it can be proved that the probability of extreme events is dominated by the minima of the action \cite{freidlin2013random} -- the trajectories that achieve such minima being referred to as instantons \cite{gurarie_instantons_1996}. Therefore, suitable numerical schemes can  be used to minimise the action and evaluate instantons as well as the related probabilities of rare events \cite{chernykh_large_2001,grafke_instanton_2013,grafke2015relevance,grigorio_instantons_2017}.
%
An obvious drawback of the aforementioned approaches is their limitation to stochastically driven dynamics which, in addition, must be simple enough so that an action may be derived and sampled, or minimised. 
%
%
...
%
In this paper, a more general approach is investigated.
\TL{Je ne comprends pas cette phrase}\EL{It relies on sampling algorithms connected to the simulation of the actual dynamics itself, \textit{i.e.} applying directly to the trajectories in the dynamic space.}
...
%
\EL{Recently, \ac{gktl} has allowed to successfully perform the numerical simulation of extreme heat waves in a simplified modelling of the atmosphere \cite{ragone_computation_2018}. 
...
The dimension of the grid is such that the size of the eddies that hit the square is comparable to its side, resulting in strong fluctuations of the drag acting on the square. 
%
...
The centerline velocity is $0.05$ in lattice unit, \textit{i.e.} normalised by $\Delta x/\Delta t$ with $\Delta x$ and $\Delta t$ referring to the lattice resolution and the time-step respectively. The distributions are imposed at equilibrium (see Appendix). 
...
\EL{The viscosity is here discarded since viscous stress is negligible as compared to pressure forces.}\TL{Redondant avec le commentaire de (1)?}
%
% typical fluctuation of the drag
%
Fig.~\ref{fig:typical_vorticity} displays the typical evolution of the vorticity field around the obstacle over a few turnover times. It is observed that, typically, some vorticity that is generated near the front part of the obstacle rapidly detaches from it and is advected downstream by the flow.
Therefore, the \textit{base pressure} varies much less than the \textit{forebody pressure} and typical drag fluctuations mostly result from pressure variation at the front of the obstacle.
...
Fluctuations appear unpredictable and the signal exhibits repeated bursts of high amplitude that deviate significantly from the averaged value.
It is therefore natural to model the drag as a scalar random process $f_d(t)$.
\begin{figure}
  \centering
  \includegraphics[width=0.7\linewidth]{typical_drag_signal/typical_drag_signal.png}
  \caption{Temporal evolution of the drag acting on the square under the action of the incident turbulent flow. The time is normalised by the turnover time related the mean-flow velocity and the size of the obstacle: $\tau_0=R/U$.}
  \label{fig:typical_drag_signal}
\end{figure}
...
It has been made possible by the relative simplicity of the investigated two-dimensional flow and the computational efficiency of the Lattice Boltzmann method. 
...
%
In that case, the \ac{pdf} is quasi-symmetric and does not display exponential tails, indicating that the asymmetry of the \ac{pdf} and the development of positive exponential tail are closely related to the no-slip condition on the obstacle surface.
\begin{figure}
  \centering
...
%
The phenomenology of extreme fluctuations of the drag is first investigated through brute-force sampling, using the control run.
Brute-force sampling is here used as opposed to approaches involving rare-events algorithms discussed in section~\ref{sec:rare_events_algorithms}.
The direct approach will provide a trustworthy baseline for the validation of rare-events algorithms. 
How rare is a fluctuation $f_d \geq a$ can be quantified by its \textit{return time} $r(a)$. This latter is defined as the average time between two consecutive occurrences of fluctuations with amplitude $f_d \geq a$\TL{, as illustrated in figure~\ref{fig:illustrate_return_time}.}
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{illustrate_return_time/illustrate_return_time}
  \caption{\label{fig:illustrate_return_time} \TL{The \textit{return time} $r(a)$ of a fluctuation $f_d \geq a$ is defined as the average waiting time between the occurrence of the event: $r(a) = \mathbb{E}[\tau]$. Note that we assume $r(a) >> \tau_0$, and therefore consider that such events are points in time.}}
\end{figure}
...
%
It is found that the base pressure variation contributes typically as $80\%$ of the overall drag fluctuation.
...
%
This high vorticity is responsible for a significant pressure drop at the base of the obstacle and, therefore, a strong drag. In contrast, nothing special happens near the forebody of the obstacle.  
...
As illustrated in Fig.~\ref{fig:density+streamlines}, this blockage is enforced by the presence of opposite vorticity in the near wake, which holds the vortex against the base of the obstacle and prevents it to be swept away.
% 
...
Since the occurrence of large drag amplitudes arises from the production of vorticity along the top or bottom side of the square, it is now proposed to characterise the dynamics of extreme events by their trajectory in the parameter space $(f_d(t), \bar{\gamma}(t))$ where $\bar{\gamma}(t)$ is the averaged shear along the top or bottom boundary of the square,
\begin{equation}
...
%
This observation may be explained by the exponential shape of the tail of the \ac{pdf} describing extreme positive drag fluctuations~\cite{lestang_article_p1}\TL{reference vers l'article JSTAT} (see Fig.~\ref{fig:pdf_drag}). 
...
As a consequence, the computational cost required to sample events of amplitude $f_d \geq a$ through brute-force sampling diverges exponentially as $e^{\lambda a}$.
%% objective
...
The motivation behind rare-events algorithms is to sample extreme fluctuations for a computational cost much lower than the related return time.
%
For this purpose, the \acl{ams} (\ac{ams}) and \acl{gktl} (\ac{gktl}) algorithms are considered in the following.
%
%
Both algorithms rely on an ensemble of $N$ trajectories $\{\mathbf{x}_n(t)\}_{0\leq t \leq T_a}$ with $n=1 \cdots N$, where each $\{\mathbf{x}_n(t)\}_{0\leq t \leq T_a}$ refers to a trajectory (of duration $T_a$) of the system in phase space. In our case, the dynamical system under investigation is the flow described in section~\ref{sec:test_flow}.
In a nutshell, trajectories are replicated or discarded in the ensemble according to some \emph{selection rules} designed to sample rare events of some observable of interest---\textit{e.g.} the drag.---with a higher frequency.
These selection rules are such that the introduced statistical bias is known at each iteration of the algorithm.
The generated trajectories are associated a weight, from which one is able to compute the probability and expectation values of observables. 
...
a \TL{sequence of short excursions that are easier to sample}~\cite{glasserman_multilevel_1999,rolland_statistical_2015}.
The related selection-mutation step of this algorithm \TL{relies on the definition of a \textit{score function} $\xi (\mathbf{x}(t),t)$} and is sketched in Fig.~\ref{fig:illustr_AMS}.
%
...
It can however be formulated in terms of the classical \ac{ams}~\cite{lestang_computing_2018} and the following results are therefore not limited to this particular variant.}
...
This allow us to highlight differences with fluid dynamics.

\begin{equation}
  \label{eq:ou}
  \dot{x} = -x + \sqrt{2\epsilon}\eta (t)  
\end{equation}
\TL{The \ac{ams} is applied  to a set of $N=32$ trajectories $\{x_n(t)\}_{0\leq t \leq T_a}$ with $T_a=5\tau_c$.
Note that, for the process defined in~\eqref{eq:ou}, the correlation time is $\tau_c = 1$.}
Our objective is to sample fluctuations $x\geq a$ with $a$ being very large compared to typical values of $x$.
The score function is simply $x(t)$ and only one single trajectory is re-sampled at each iteration.
%
...
Obviously, the successive re-samplings of the \ac{ams} algorithm lead very rapidly to trajectories exhibiting extreme fluctuations.
The related computational cost is many orders of magnitude lower than that of direct sampling.

Undoubtedly, the \acl{ou} process is an oversimplified dynamics to showcase the efficiency of the \ac{ams} algorithm.
Indeed, the state space is one-dimensional and the choice of the score function is straightforward: it is $x$ itself.
...
However, a natural simple choice for which we will opt for in this study is $\xi (\mathbf{x}(t),t) = f_d(t)$. 
\TL{Let us stress here that the flow dynamics are deterministic.
Randomness must therefore be artificially introduced for the re-sampling procedure to be effective, \textit{i.e.} to generate new trajectories associated to larger drag fluctuations.
In practice we introduce a small perturbation in the state of the flow at the branching points (see figure~\ref{fig:illustr_AMS}).
Due to the chaoticity of the flow, re-sampled trajectories will then separate from their parent.
The timescale over which two trajectories separate due to an infinitesimal perturbation of the initial state is known as the \textit{Lyapunov timescale}.
It is denoted $\tau_L$ in the remainder of this paper.
We make the important assumption that such a perturbation at branching times has a negligible impact on the properties of the sampled rare events.
The validity of this assumption can be verified by performing a brute force simulation of the dynamics over a long duration, regularly perturbing the state of the flow.
...
% application
%
The \ac{ams} is applied with $N=256$ trajectories of duration $T_a = 20\tau_c$.
%
...
Importantly, one can see that re-sampled trajectories never exceeds the amplitude of the highest maximum in the initial set of trajectories. This contrasts with the case of the \acl{ou} process (Fig.~\ref{fig:comparaison_temps_de_retour}). 
...
\begin{figure}
  ...
  The \ac{ams} fails to efficiently sample rare trajectories. All trajectories are eventually re-sampled from the trajectory displaying the highest maximum in the initial ensemble.}
\end{figure}
...
Because the dynamic process has a memory, it takes a certain time $\tau_L$ before a re-sampled trajectory separates from its parent. In our situation, this memory originates from the deterministic nature of fluid dynamics. 
%
\TL{Importantly, we observe in Fig.~\ref{fig:AMS_drag_trajectories} that the duration of extreme drag fluctuations is shorter than the Lyapunov timescale $\tau_L$ of the drag process.}
As a consequence, the re-sampling of a trajectory branched close to $t=t^{\star}$---the maximum drag amplitude---cannot lead to larger values for $t^{\star} \leq t \leq t^{\star}+\tau_L$.
For $t - t^{\star} >\tau_L$, the drag process has lost the memory of the drag fluctuations on which the re-sampling was based.
Therefore, the probability of observing a new extreme fluctuation is also very low.
%
...
% perspectives
%
In summary, it appears that a straightforward application, defining the score function as the drag itself, does not allow us to efficiently sample extreme fluctuations.
%
...
The \ac{ams} algorithm could be used in the same way as in section~\ref{sec:ams_drag}, \textit{i.e.} by setting $F_T$ as the score function $\xi$.
However, this leads to similar unsatisfactory conclusions.
...
%
The weights are chosen so that, after several iterations of the algorithm, the trajectories in the ensemble are distributed according to a biased probability distribution that favours trajectories related to large values of the (time-averaged) observable of interest.
%
...
Similarly to the \ac{ams} algorithm, the \ac{gktl} algorithm is based on the simulation of an ensemble of $N$ trajectories $\left\{\mathbf{x}_{n}(t)\right\}_{0\leq t \leq T_a}$ with $ n =1 \cdots N$ starting from independent random initial conditions.
%
...
%
%
\TL{The weights $\{W_{n}^{i}\}_{n=1\cdots N}$ determine how many copies of each trajectory are produced at time $t=t_i$. See~\cite{brewer2018efficient, lestang:tel-01974316} for more information on the practical implementation of the algorithm.
The application of this re-sampling at each step $t_i$ eventually leads to a biased sampling of trajectory space, for which trajectories corresponding to extreme values of $\int_{0}^{T_a}A(\mathbf{x}_{n}(t))dt$ have a higher probability.
...
However, similarly to the case of \ac{ams}, randomness must be artificially introduced for the cloning procedure to be effective for deterministic dynamics and we proceed in the same way as as described in section~\ref{sec:ams_drag}.}
...
In practice we set $N=16384$ and $T_a = T = 10\tau_c$, leading to $C_{gktl} \approx 1.6 \times 10^5 \tau_c$.
...
As a result, a rule of thumb is to set $\tau \approx \tau_c$.
In the following experiments we set $\tau = \tau_c /2$.
%
...
%
%
Fig.~\ref{fig:IS_GKTL} displays the estimate of the biased \ac{pdf}s of the (zero-mean) time-averaged drag, $\tilde F_T \equiv F_T - \overline{F_T}$:
\begin{equation}
...
%
Indeed for $k \gtrsim k_{max}$, the re-sampling relies on a small number of ``independent trajectories'', as most of the trajectories (in the biased ensemble) overlap. 
This effect is highlighted in Fig.~\ref{fig:IS_GKTL} where the biased \ac{pdf} corresponding to $k=0.03$ is over-peaked\TL{Anglais?}.
%
Such a finite-size effect is expected to gradually increase as the bias amplitude  $k$ is increased.
%
An important question concerning the application of the \ac{gktl} algorithm is therefore that of the dependence of $k_{max}$ on the ensemble size $N$, addressed in recent studies~\cite{nemoto2017finite,Guevara_Hidalgo_2018}.
...
\begin{table}
	...
	\caption{Number of fluctuations $F_{T} \geq a$ with a=$\sigma,~2\sigma,~...$ captured in the biased ensemble for $k=0.02,~0.025$ and $0.03$.
		...
		${N}_{brute-force}$ is the number of fluctuations expected from the direct sampling of $N$ independent realisations of $F_{T}$ of the same duration.}
...
Trajectories that overlap for more than half of their duration are counted as one.
%\footnote{Trajectories that overlap over an interval $[O;t]$ with $t\geq T_a / 2$.}
This number is compared to the expected number of fluctuations obtained from simulating $N$ independent trajectories $\{F_T(t)\}_{0\leq t \leq T_a}$, \emph{i.e.} with the same computational cost $C_{gktl}$.
Table~\ref{tbl:nbTraj_tbl} shows that the \ac{gktl} algorithm is able to sample drag fluctuations that are far from reach from a brute-force sampling approach of similar computational cost.
%
...
Statistics for extreme events can be computed from the biased ensemble by inverting eq.~\eqref{eq:Biased_Path_Approximation}.
For instance, it is possible to compute return times (see fig.~\ref{fig:illustrate_return_time}) for
events unreachable by a direct approach.

In the following we use the \ac{gktl} algorithm with $N$ trajectories of duration $T_a > T$.
Each trajectory in the biased ensemble results in a timeseries for the time-averaged drag:
\begin{equation}
  \label{eq:time_averaged}
  F_T(t) = \int_{t-T}^{t}f_d(\tau)d\tau, \quad t\in [T,T_a]  
\end{equation}

The return time of a fluctuation $F_T \geq a$ is given by~\cite{lestang_computing_2018}
\begin{equation}
r(a) = - \frac{T_a - T}{\ln (1-p(F_T \geq a))}.
\end{equation}
The probability $p(F_T \geq a)$ can be estimated from the biased ensemble, inverting eq.~\eqref{eq:Biased_Path_Approximation}:
\begin{equation}
p(f_d \geq a) \approx \frac{1}{N}\sum_{j=1}^{N}e^{T_a \lambda(k)}e^{k \int_{0}^{T_a}f_d(t)dt}s_j(a),
\end{equation}
with $s_j(a) = 1$ if $\max_{T\leq t \leq T_a}[F_T^{(j)}] \geq a$ and $s_j(a) = 0$ otherwise.
That is, summing the weights of the timeseries which maximum is above $a$.

Figure~\ref{fig:return_times_gktl} displays the return times for extreme fluctuations of the time-averaged drag acting on the square obstacle.
It comapres return times plots for two values of the bias $k$ and a fixed computational cost $N \times T_a$ to the return times obtained from a timeseries of duration $T_a = N \times T_a$.
While a direct approach cannot access events with a return time greater than $T_{tot}$, the \ac{gktl} algorithm allows for the computation of statistics for drag fluctuations having a return time several order of magnitude above the computational cost.

\begin{figure}
  \centering
  \includegraphics[width=.7\linewidth]{return_times_GKTL/return_times_GKTL}
  \caption{\label{fig:return_times_gktl} Return times for the time-averaged drag acting on the square obstacle. $\tilde{F}_T$ denotes the time-averaged drag with zero mean.
    The continuous lines are obtained based on the biased ensemble of trajectories generated by the \ac{gktl} algorithm using $N=1024$ and $T_a=30\tau_c$. The dashed line is the average return time plot obtained from a timeseries of duration $T_{tot} = N\times T_a$.}
\end{figure}
...
This can be related to the phenomenology of extreme drag fluctuations, which lifetime is shorter than the timescale over which re-sampled trajectories separate from their parent following the
...
In this case, we showed that using the \ac{gktl} leads to a tremendous improvement with respect to a brute-force approach, allowing the simulation and computation of statistics for trajectories that are out of reach without algorithm.
The sampling of extreme time-averages is aided by the selection of trajectories displaying an exceptional succession of drag fluctuations, resulting in a an extreme value for the average.
It is however unclear whether the \ac{gktl} is suited to sample trajectories for which the extreme fluctuations of the time-averaged drag results from a unique, exceptionally large drag fluctuation.
\\
Successful usage of the \ac{ams}, \ac{gktl} and similar methods for complex flows relevant to industrial or environmental situations will require coping with the fact that quality score functions
