...
\label{sec:ams}
The \acl{ams}~\cite{amspaper} inherits from the base ideas of \textit{splitting} algorithms~\cite{kahnharris}: the sampling of a rare event can be made easier by splitting the event
into a sequence of events that can be sampled with a higher probabilbity.
...
In recent years, it was applied to several problems
...
In the remainder of this section, we illustrate that the application of the \ac{TAMS}, fails to easily and efficiently sample rare drag fluctuations occuring in the test flow described in section~\ref{sec:test_flow}.
We analyse those difficulties in light of the conclusions of sections~\ref{sec:instantaneous_drag} and~\ref{sec:time_avg} on the dynamics of extreme drags.

To begin with, figure~\ref{fig:comparaison_temps_retour} illustrate expected results from the \ac{tams}, based on a one-dimensional \ac{ou} process.
In this case, it is clear that the algorithm allows for the sampling of very rare fluctuations for a computational cost that is many order of magnitude lower than their return time.

We then apply the \ac{tams} to the test flow, to sample extreme drag fluctuations on the square obstacle. 
