...
Many of them involve keeping track of an ensemble of $N$ trajectories $\{x_n(t)\}_{n}$.
...
In other words, generated trajectories are given a weight which knowledge allow for the computation of their probability and expectation values according to the unbiased stationary distribution of the dynamics.
...
\label{sec:ams}
% Introduction to AMS
The \acl{ams}~\cite{amspaper} inherits from the base ideas of \textit{splitting} algorithms~\cite{kahnharris}: the sampling of a rare event is made easier by splitting the event into a sequence of events that can be sampled with a higher probabilbity.
Even though a large body of variants have been proposed since the early 50s, the \ac{ams} appears as the most advanced formulation of multilevel splitting to date.
In recent years, it was applied to several problems, ranging from the computation of the dissociation time of biomolecules~\cite{teo2016} to the simulation of rare relaminarisations in a stochastic model of wall turbulence~\cite{rolland2017}.

% Quick description of TAMS
In the present paper we use a variant of the \ac{ams} algorithm called the \ac{tams}.
The \ac{ams}, and therefore the \ac{tams}, relies on the definition of a \textit{score function} $\xi (\mathbf{x}(t))$.
It is an observable of the dynamics that acts as a measure of the distance of the current state $\mathbf{x}(t)$ to the region of the state space that correspond to the rare event(s) of interest.
The \ac{ams} algorithm is an iterative procedure, that starts from an ensemble of $N$ independant trajectories of the dynamics over a time-interval $[0;T_a]$.
At each iteration, trajectories with the lowest maxima of the score function over $[0;T_a]$ are discarded and replaced by trajectories that achieved a higher value of the score function at some point of their
history,
The algorithm is sketched in figure~\ref{figure_AMS}.
For further details about the algorithm and assoiated mathematical results, see~\cite{lestang2018} and references therein.

% Example: OU process
We first describe the result of the \ac{tams} on a simple case.
We consider a one-dimensional \ac{ou} process
\begin{equation}
  \label{eq:ou}
  \dot{x} = -x + \sqrt{2\epsilon}\eta (t)  
\end{equation}

and apply the \ac{tams} to simulate trajectories $\{\mathbf{x}(t)\}_{0\leq t \leq T_a}$ with $\underset{0\leq t \leq T_a}{\max} x(t) \geq a$ with $a$ very large with respect to typical fluctuations of $x(t)$.
Figure~\ref{fig:compraison_temps_de_retour} illustrate the amplitude of the maximum over trajectories along the iterations of the algorithm.
...
Figure~\ref{fig:compraison_temps_de_retour} illustrates that the \ac{tams} samples extreme fluctuations $x \geq a$ for a computational cost several order of magnitude lower than the return time of such fluctuations.
...
In addition, the noise term in~\eqref{eq:ou} has no correlations in time, which means that newly generated trajectories will quickly separate from their ancesotrs.

% Application to turbulent flow past an obstacle
Can we obtain similar results by applying the \ac{tams} to the test flow described in\ref{sec:test_flow} ?n
The objective is to sample trajectories that display extreme fluctuatons of the drag $f_d$ acting on the square obstacle. However, by contrast with a simple \ac{ou} process, the phase space in higly-dimensinal and the choice of the score function $xi (\mathbf{x}(t))$ is not so clear.
In the following we use the \ac{tams} with $\xi \equiv f_d$, which is the most simple choice for the score function.
For such complex dynamics however, there is no resons for the drag itself to be a good measure of how far is a given state in phase space to regions where the drag is extreme.
Unfortunately, the choice of better score functions rely on a deep understanding of the physics of the rare events of interest, whcih is precisely what we are tryping to achieve.

% Commentaire figure instant_256_20

Figure~\ref{fig:instant_256_20} displays similar information as figure~\ref{fig:comparaison_temps_de_retour} for $N=256$ initial trajectories over $T_a = 20 \tau_c$.
Additionally, figure~\ref{fig:instant_256_20} shows the distribuion of the maximum of the drag among the initial trajectories.
...
However, the maximum over the resampled trajectories never exceeds the amplitude of the highest maximum in the initial set of trajectories.

% Commentaire de la figure iter_181

Figure~\ref{fig:instant_256_20} can be better understood by visualizing the set of the last resampled trajectories. 
One can see that, ultimately, trajectories have been resampled from the same initial trajectory.
We explain this phenonemon as follows.
Because the flow dynamics hae a memory, it takes a certain time before a resampled trajectory separate from its parent.
This is clearly visible on figure~\ref{fig:instant_256_20}.
In our case, this memory originates from the deterministic nature of the dynamics, which implies that two trajectories starting from infinitesimally close starting point will overlap over a timescale called
the <<Lyapunov timesacle>>~\cite{lyapunovRef}.
However, we observe that the duration of extreme drag fluctuations is typically 5 times smaller that the Lyapunov timescale of the drag proces.
...
For $t > \tau_L$, the drag process has no memory of the drag fluctuations on which the resampling was based and the probability of observing an extreme fluctuation over $\tau_l \leq t \leq T_a$ is very low.
...
Indeed, fluid structures responsible for a particular value of the drag on the obstacle at a time $t^{\star}$ are swept away by the mean flow over a timescale $\tau_c$.

% perspectives

\begin{itemize}
\item more trajectories
\item different score functino?
\item Change algorithm
\end{itemize}
\begin{figure}
  \centering
  \includegraphics[width=.7\linewidth]{ams/figure_AMS}
...
\begin{figure}
  \centering
  \includegraphics[width=.7\linewidth]{ams/comparaison_temps_de_retour}
...
\begin{figure}
  \centering
  \includegraphics[width=.7\linewidth]{ams/instant_256_20}
...
\begin{figure}
  \centering
  \includegraphics[width=.7\linewidth]{ams/iter_181}
...
The \ac{ams} could be used in the same way as described in section~\ref{sec:ams} for the instantaneous drag, choosing $F_T$ itself as a score function.
However, we observed a similar behaviour: the \ac{tams} resampling procedure fails to yield trajectories with higher fluctuations of $F_T$.

When it comes to time-averaged observable, one can make use of a different algorithm the \acf{gktl} algorithm~\cite{Giardina2006,Tailleur2007,Giardina2011}.
It was originally designed to compute large deviations of time-averaged dynamical observables.
By contrast with the \ac{ams} and \ac{tams}, the \ac{gktl} algorithm does not belong to the family of splitting algorithms.
...
Similarly to the \ac{tams} algorithm, the \ac{gktl} relies on the simulation of a population of trajectories.
...
The weights are chosen such that, after several iterations of the algorithm, generated trajectories are distributed according to a probability distribution that is tilted in order to favour trajectories with large values of a chosen time averaged observable.
This sort of algorithm has first been proposed by~\cite{Giardina2006} and has been used to study rare events in both stochastic~\cite{Giardina2006,Lecomte2007b,Garrahan2007,Hurtado2009b} and deterministic systems~\cite{Giardina2006,Tailleur2007}.

More precisely, we perform simulations of an ensemble of $N$ trajectories $\left\{ X_{n}(t)\right\}$ (with $n=1,2,...,N$) starting from random initial conditions.
Like in section~\ref{sec:ams}, the total integration time of the trajectories is denoted $T_{a}$.
We consider an observable of interest $A(X(t))$ and a resampling time $\tau$.
...
\end{equation}
For each trajectory $X_{n}$, a random number of copies of the trajectory are generated, on average proportional to the weight $W_{n}^{i}$ and such that the total number of trajectories produced at each event is equal to $N$.
...
The parameter $k$ is chosen by the user in order to control the strength of the selection and thus to target a class of extreme events of interest.
The larger the value of $k$, the more trajectories with large values of the time average observable will survive the selection.
...
We now discuss the application the \ac{gktl} algorithm to the flow dynamics introduced in section~\ref{sec:test_flow}.
...
Consistenly with section~\ref{sec:time_avg}, we choose the averaging window $T=10\tau_c$.

% Choice of parameters and perturbation
The computational cost $C_{gktl}$ of a \ac{gktl} experiment is determined by both the duration of the trajectories $T_a$ and the number of trajectories $N_c$: $C = N_c \times T_a$.
In the following series of experiements we set $T_a = T = 10\tau_c$.
The sampling from the \ac{gktl} alorithm will be compared to the sampling from a direct simulation of the flow over $T_{tot}=10^6\tau_c$.
To match this computational cost with the \ac{gktl} experiments, we set $N_c=16384$.
Lastly, one must set the cloning period $\tau$.
On the one hand, too small a cloning period may result in unfortunate loss of information as clones do not separate from their parent over a cloning period.
On the other hand, choosing $\tau \gg \tau_c$ leads to too few cloning stages.
As a consequence, we set $\tau = \tau_c /2$.

% Commentaire de la figure illustration_IS


Figure~\ref{fig:illustration_IS} illustrates the importance sampling performed by the \ac{gktl} algorithm.
illustrates the amplitude of the fluctuations of $F_{T_a}$ corresponding to the sampled trajectories for each three experiments.
More precisely, it illustrates the estimate of the biased drag \ac{pdf} $\rho_k$ in each case:
\begin{multline}
...
\end{multline}
Moreover, figure~\ref{fig:illustration_IS_drag} displays an estimate of the unbiased drag \ac{pdf} $\rho_0(F) = \mathbb{E}_{\set{P}_0}[\delta(F_{T_a} - F)]$.
Note that this figure is analogous to figure~\ref{fig:illstration_importance_sampling} on page \pageref{fig:illstration_importance_sampling}.
One can see that, as $k$ is increased, the biased \ac{pdf} is shifted towards extreme values.
However, the experiment with $k=0.03$ does not seem to increase the sampling of rare event with respect to the experiment with $k=0.025$.
Actually, the shape of the estimate of the biased \ac{pdf} shows that the reconstructed trajectories concentrate around a single value of the fluctuation $F'_{T_a} = F_{T_a}-\overline{F_{T_a}} \approx 5\sigma_{T_a}$, where $\overline{F_{T_a}}$ and $\sigma_{T_a}$ denote the average and standard deviation of $F_{T_a}$, respectively.
This suggests a strong overlap of trajectories in the reconstructed ensemble, which we address next.

\paragraph{}
Figure~\ref{fig:nbIndep} illustrates the overlap of trajectories in the reconstructed ensemble.
Along the X-axis, we represent the $M = T_a / \tau$ iterations of the evolution/selection procedure.
For each iteration $m$ we plot the number of independent trajectories, \textit{i.e.} the number of trajectories that exhibit different paths $\{X(t)\}_{(m-1)\tau \leq t \leq m\tau}$ over the interval $t\in [(m-1)\tau;m\tau]$.
In practice we trace back the history of the trajectories backward in time starting from the final time $T_a = M\tau$, in a way very similar to the \emph{Method 1} presented in section~\ref{sec:reconstruction_practice}.
Figure~\ref{fig:nbIndep} illustrates that, at early times, the vast majority of the trajectories in the reconstructed ensemble overlap.
This is also illustrated, in a different way, by the red trajectories in figure~\ref{fig:schema_algo_b}.
Say $N$ trajectories of the reconstructed ensemble overlap over the interval $[0;m\tau]$, with $1\leq m \leq M$.
Among these $N$ trajectories, only one trajectory $j$ actually originates from $t=0$.
Indeed, the $N-1$ others have been discarded following the selection stage of a given iteration $\tilde{m}\geq m$, and replaced by a clone of trajectory $j$ over the interval $[0;\tilde{m}\tau]$.
In the following we call \emph{fully independent trajectories} the trajectories that do not overlap at all over $[0;T_a]$, \textit{i.e.} trajectories that do not overlap over the first period $[0;\tau]$.
From figure~\ref{fig:nbIndep} one can see that the number of fully independent trajectories is very small compared to the total number of trajectories $N_c$.

\paragraph{}
In this context, can the \ac{gktl} yield more \emph{independent} events than a direct sampling based on a long simulation ?
...
\begin{table}
  ...
  \end{tabular}}\\
\subfloat[$k=0.025$]{\label{tbl:nbTraj_tbl_b}
  \begin{tabular}{l|ccccc}
    Overlap & $\sigma$ & $2\sigma$ & $3\sigma$ & $4\sigma$ & $5\sigma$ \\
    \hline
    0\% & 305 & 179 & 52 & 5 & 0  \\ 
    ...
    $N_{N_c}^{(gauss)}$ & 2599.408 & 372.738 & 22.117 & 0.519 & 0.005  \\ 
  \end{tabular}}\\
\subfloat[$k=0.03$]{\label{tbl:nbTraj_tbl_c}
  \begin{tabular}{l|ccccc}
    Overlap & $\sigma$ & $2\sigma$ & $3\sigma$ & $4\sigma$ & $5\sigma$ \\
    \hline
    0\% & 100 & 80 & 33 & 8 & 1  \\ 
    ...
    $N_{N_c}^{(gauss)}$ & 2599.408 & 372.738 & 22.117 & 0.519 & 0.005  \\ 
  \end{tabular}}
\caption{Number of independent trajectories corresponding to fluctuations $F'_{T_a} \geq a$ with a=$\sigma,2\sigma...$ in the reconstructed ensemble, for the three experiments $k=0.02,0.025,0.03$. Parameters of the \ac{gktl} experiments are $N_c = 16384$, $T_a=10\tau_c$, $\tau = \tau_c /2$ and $\epsilon = 0.02$. In each case we count as independent trajectories with an overlap of $0\%$ (zero overlap), $25\%$ and $50\%$, respectively. In addition, $N_{N_c}^{(gauss)}$ is the average number of fluctuations $F'_{T_a} \geq a$ on can expect from $N_c$ realisations obtained from a direct sampling approach. This number is based on the fact that the \ac{pdf} of $F_{T_a}$ is Gaussian. It is defined in~\eqref{eq:gaussian_number}.}
\label{tbl:nbTraj_tbl}
\end{table}
Table~\ref{tbl:nbTraj_tbl} displays the number of independent trajectories that correspond to fluctuations $F'_{T_a} \geq a$ with $a=\sigma_{T_a}, 2\sigma_{T_a},3\sigma_{T_a}...$, for each of the three experiments.
In each case, we count the number of trajectories according to the allowed degree of overlap.
For instance, allowing a $25\%$ overlap, we consider trajectories that overlap over $[0;T_a/4]$ as independent.
In order to compare with a direct sampling approach, we could perform a very long simulation of computational cost $N_c\times T_a$, divide it into $N_c$ realisations $F_{T_a}$ and count the number of fluctuations $F'_{T_a} \geq a$ above a given threshold value $a$.
However, we found that the \ac{pdf} describing the statistics of fluctuations $F'_{T_a}$---with $T_a = 10\tau_c$--- is very close to a Gaussian \ac{pdf}.
As a result, the expected number of fluctuations $F'_{T_a} \geq a$ among $N_c$ realisations is
\begin{equation}
  N^{(gauss)}_{N_c} \approx N_c \times \set{P}(F'_{T_a} \geq a) = N_c \times \frac{1}{\sqrt{2\pi \sigma_{T_a}}}\int_{a}^{\infty}e^{-x^2/2\sigma^2_{T_a}}\mathrm{d}x.
  \label{eq:gaussian_number}
\end{equation}
Considering small fluctuations $F'_{T_a} \geq a$ with $a = \sigma$ and $a = 2\sigma$, one can see in table~\ref{tbl:nbTraj_tbl} that the number of sampled independent trajectories actually decreases as $k$ is increased. This results from the fact that the overall number of independent trajectories decreases as $k$ is increased, as illustrated in figure~\ref{fig:nbIndep}.
More importantly, considering fluctuations above $3\sigma$, the \ac{gktl} experiments for $k=0.025$ and $k=0.03$ provide a larger number of fluctuations than a direct sampling approach with similar computational cost.
For instance, table~\ref{tbl:nbTraj_tbl_b} shows that the experiment with $k=0.025$ provides $5$ fully independent trajectories corresponding to fluctuations $F'_{T_a} \geq 3\sigma$.
This is ten times more than what can be expected from a direct sampling.
Notice however how drastic is the reduction of the number of independent trajectories as the allowed degree of overlap is diminished.
Allowing for a $50\%$ overlap, the reconstructed ensemble for the experiment $k=0.025$ contains roughly $200$ independent trajectories corresponding to fluctuations $F'_{T_a} \geq 3\sigma$.
Generally speaking, we observe in all three experiments that the number of independent trajectories is very small compared to the total number of trajectories $N_c = 16384$.
For instance, table~\ref{tbl:nbTraj_tbl_c} shows that, allowing a $50\%$ overlap between independent trajectories, $521$ trajectories can be used to study the dynamics leading to fluctuations $F'_{T_a} \geq 3\sigma$.
This represents $521 / N_c \approx 3\%$ of the overall computational cost of the experiment.
We stress that, in spite of this rather poor yield, the \ac{gktl} still results in a larger amount of extreme fluctuations, with respect to a direct sampling approach.
However, there is clearly room from improvement.
