...
parser.add_argument("directory", type=str, help="Location of GKTL directory relative to the current directory.")
...
for counter, common_ancestor_idx in enumerate(dict_of_parents):
    ...
    for jj in group_of_trajs:
        fileName=join(args.directory,"recon_rep_0_clone_{}.traj".format(jj))
        # Reads instant. drag
        drag_maxima_ingroup.append(np.amax(np.fromfile(fileName, float, -1, "")))
    
    j = group_of_trajs[np.argmax(drag_maxima_ingroup)] # Pick traj with the largst maximum in group
    traj_idx.append(j)
    drag_maxima.append(np.amax(drag_maxima_ingroup))

with open("extremes.csv", "w") as f:
    for idx in np.argsort(-np.array(drag_maxima)):
