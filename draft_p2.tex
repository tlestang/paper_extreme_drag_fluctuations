...
\begin{figure}
  ...
  \textbf{(b)} Autocorrelation function of the drag defined as $C(\tau) = \overline{ \tilde f_d(t+\tau)\tilde f_d(t)} ~/~ \overline{{\tilde f_d}^2}$. The correlation time $\tau_c$ is given by $C(\tau_c)=0$. \EL{mettre $\tau/\tau_c$ en abscisse de l'autocorrelation (b)}
...
How rare is a fluctuation $f_d \geq a$ can be quantified by its so-called \textit{return time} $r(a)$. This latter is defined as the average time between two consecutive occurrences of events of amplitudes $f_d \geq a$. \EL{suffisamment clair? un schema pour fixer les choses?}
