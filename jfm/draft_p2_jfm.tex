...
The aim of rare-event sampling algorithms is to sample extreme fluctuations for a much lower computational cost.
...
\ZZ{Rare event algorithms compute an ensemble of $N$ trajectories $\{\mathbf{x}_n(t)\}_{0\leq t \leq T_a}$ with $n=1 \cdots N$.
...
trajectories with a fixed duration $T_a$~\citep{lestang_computing_2018}.%
...
We use the observable itself as the score function : $\xi (\mathbf{x}(t),t) = f_d(t)$.  
%
...
% (see Fig.~\ref{fig:illustr_AMS} and~\cite{wouters2016rare}).
	%
Due to the chaotic dynamics, re-sampled trajectories separate rapidly from their parents, on a timescale of the order of the Lyapunov time.}
...
\begin{figure}
	...
	\caption{\label{fig:AMS_drag_resampling} Maxima of the instantaneous drag throughout $256$ re-sampled trajectories (vertical axis) as a function of the corresponding \EL{computational cost $C_{AMS}$} in correlation-time unit. The \ac{ams} algorithm fails to efficiently sample rare trajectories associated to drag fluctuations higher than the largest fluctuation already captured in the initial ensemble. \QQ{Change TAMS to AMS in horizontal axis}}
\end{figure}


%pitfall
%
\ZZ{Fig.~\ref{fig:AMS_drag_trajectories} displays the ensemble of drag values after many iterations of the \ac{ams} algorithm.} 
%
Interestingly, one obtains that all trajectories are eventually re-sampled from the same trajectory displaying the highest maximum in the initial ensemble.
%
...
We observed that the trajectories with the lowest maxima of the score function are discarded after a few iterations, and new trajectories with higher maxima are re-sampled. 
%Figure~\ref{fig:AMS_drag_resampling} shows the evolution of the maxima of the re-sampled trajectories as the \ac{ams} is iterated, \textit{i.e.} as a function of the computational cost.
...
Our observable of interest is therefore the drag $f_d(t)$ and the duration $T_a$ of each trajectory corresponds to the period of averaging of the drag, \emph{i.e.} $T_a=T$.    
...
%
\EL{In brief, trajectories are first evolved independently up to time  $\tau < T_a$ with $\tau$ referring to a cloning period. Then, the selection-and-cloning rules apply according to the average of the observable of interest ($f_d(t)$) over the interval $[0,\tau]$ (see Appendix \ref{app:gktl_description} for details). 
...
\EL{The algorithm depends on three parameters: the cloning strength $k$, the number of trajectories $N$ and the resampling period $\tau$. The higher $k$, the larger is the bias involved in the statistical resampling, as illustrated in Fig.~\ref{fig:IS_GKTL}. As for the \ac{ams} algorithm, $N$ governs the error affecting the averaged quantities evaluated from the biased ensemble of trajectories, and should be chosen as high as possible. Finally, the cloning period $\tau$ determines how often the resampling is performed. The choice of the cloning period $\tau$ is important in practice.
...
As a result, a safe rule of thumb is to take $\tau$ of the order of $\tau_c$. In the following experiments, $\tau = \tau_c /2$.}
