...
\ZZ{The next two paragraphs may be moved to the introduction of section \ref{sec:rare_events_algorithms}?}
\smallskip
...
\EL{In that case, the PDF of $F_T$ is found nearly Gaussian as a consequence of the Central Limit theorem (see Fig.~\ref{fig:IS_GKTL}).}
...
\ZZ{ The next section has been strongly reduced by keeping only key results accessible to broad JFM audience. Technical details are either removed (reference to a companion paper) or moved to Appendix}
...
\ZZ{Should develop here the idea behind rare event sampling. How it works. Include here the paragraphs of the introduction. Justify the choice of ams and gktl: what are differences and complementarity. AMS = instantaneous signal, GKTL= time-averaged signal. Why are they representative? What are the expectations?}
...
%In a nutshell
Trajectories are replicated or discarded in the ensemble according to some \emph{selection rules} designed to sample rare events of some observable, \textit{e.g.} the drag, with a higher frequency.
...
The sampling of a rare event is made easier by splitting the dynamical path of the system (in phase space) into
%a sequence of events that can be sampled with a higher probability
a sequence of shorter excursions \citep{glasserman_multilevel_1999,rolland_statistical_2015}.
...
{Let us mention that we use a specific form of the \ac{ams}, in which trajectories have a fixed duration $T_a$. It can however be formulated in terms of the ``classical \ac{ams}''~\citep{lestang_computing_2018}.
The following results are not limited to this particular variant.}

\bigskip

\ZZ{The Ornstein-uhlenbeck application has been moved to appendix \ref{app:AMS_on_OU}. Enough to mention in the text that AMS works successfully for OU and refer to apppendix?}
...
\begin{figure}
	...
	\caption{\label{fig:AMS_drag_resampling} Maximum of the instantaneous drag throughout the re-sampled trajectories as a function of the corresponding \EL{computational cost $C_{AMS}$}. The \ac{ams} is unable to efficiently sample rare trajectories associated to drag fluctuations higher than the largest fluctuation already captured in the initial ensemble.}
\end{figure}
...
%
Our aim is here to use the \ac{ams} algorithm to sample flow evolutions that exhibit extreme fluctuations of the drag $f_d$ acting on the square obstacle.
...
\ZZ{Should insist on the efficiency of GKTL to compute return times. The formula may be moved to the appendix?}
\bigskip
