...
Indeed, fluid structures responsible for a particular value of the drag on the obstacle at a time $t^{\star}$ are swept away by the mean flow over a timescale $\tau_c$, shorter than the time it takes for
...
This parameter sets the amplitude of the typical fluctuations within the biased ensemble sampled by the algorithm.
In other words, a higher value of $k$ will give a higher weight to higher fluctuations, thus driving the sampling to more extreme events.
...
In the latter case, the parameter $k$ can be related to the typical amplitude of the sampled flucutations through the Gartner-Ellis theorem~\cite{GEref}.
...
Figure~\ref{fig:illustration_IS} illustrates importance sampling achieved with the \ac{gktl} algorithm for the time-averaged drag $F_T$ with $T = T_a = 10\tau_c$.
For each three experiment, it displays the estimate of the biased \ac{pdf}
\begin{equation}
...
It corresponds to equation~\eqref{eq:estimate_biased_measure} with $k=0$.
...
Setting $k$ to a high value causes finite population effects: trajectories overlap over most of their duration.
...
Unsurprisingly, sampling larger fluctuations thus reauire a larger number of populations, and therefore a larger computational cost.
...
As a comparison, it also displays the average number of such fluctuations in a long timeseries of duration $T_{tot}=10^6\tau_c$, divided in $T_{tot} / T_a$ events.
This number is computed based on the assumption that the \ac{pdf} describing the random variable $F_T$ with $T=10\tau_c$ is Gaussian, motivated by figure~\ref{fig:pdf_averared}.
...
\begin{table}
  ...
  \caption{Number of independent corresponding to fluctuations $F_{T_a} \geq a$ with a=$\sigma,2\sigma...$ in the biased ensemble, for the three experiments $k=0.02,0.025,0.03$. Parameters of the \ac{gktl} experiments are $N_c = 16384$, $T_a=10\tau_c$, $\tau = \tau_c /2$. Within the biased ensemble, trajectories are considered distinct if they overlap for less than $50\%$ of their duration.
    In addition, $\mathcal{N}_{direct}$ is the average number of fluctuations $F'_{T_a} \geq a$ on can expect from $N_c$ independant realisations of $F_{T_a}$, \textit{i.e.} with $k=0$.
