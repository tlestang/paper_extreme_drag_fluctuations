...
\begin{figure}
	...
	\caption{Instantaneous drag signals $f_d(t)$ corresponding to the highest fluctuations of the time-averaged drag $F_T$ with averaging period $T=10~\tau_c$ and $\tau_c$ being the correlation time of the drag. $\sigma_T$ denotes the standard deviation of $F_T$. \EL{FIGURE IS MISSING} \TL{Addressed in issue \#9}}
...
\begin{figure}
  ...
  \caption{\label{fig:illustr_AMS} Selection-mutation procedure of the \ac{ams} algorithm to compute the probability that a real-valued observable $A(\mathbf{x}(t))$ reaches values larger than $Q$ throughout a trajectory of duration $T_a$.\EL{QUID of the score function mentioned in the text: is the observable? \#1} The initial ensemble of blue trajectories (associated with the time evolution of the observable $A$) is first generated. These trajectories are independent and can be simulated in parallel. On the basis of the respective maxima $Q_1$, $Q_2$ and $Q_3$, the trajectory with the lowest maximum is discarded in the ensemble (dashed blue line). Among the two remaining trajectories, trajectory 3 is chosen randomly and copied until it reaches the value $Q_1$. It is then freely simulated from the branching point to the final time $T_a$. In case of deterministic dynamics, a small perturbation is introduced at the branching to separate the trajectories. This procedure is iterated $J$ times or until all trajectories exceed a fixed threshold $Q$.}
\end{figure}
...
\TL{For a recent review of the \ac{ams}, its history and applications, see~\cite{cerou_2019}.}
\TL{Note that we use a specific form of the the \ac{ams} in which trajectories have a fixed duration $T_a$.
It can however be formulated in terms of the classical \ac{ams}~\cite{lestang_computing} and the following results are not limited to this particular variant.}

\subsubsection{Application of the \ac{ams} to a simple case: the \acl{ou} process}
\label{sec:ams_ou}
...
As a result, resampled trajectories do not differ from the parent trajectory their are resampled
from.
A workaround is to introduce a slight perturbation in the state of the flow at the branching point (see figure~\ref{fig:illustr_AMS}).
Because of the flow chaoticity, resampled trajectories will separate from their parent over the \textit{Lyapunov timescale} $\tau_L$.}\EL{Freddy: provide reference for Lyapunov timescale. \#12}
...
Similar ideas have been applied in a wide range of fields over the past 50 years, under different names \EL{donner des exemples \#3}, depending of the specific application domain~\cite{moral_feynman-kac_2004}.
...
%
%
\TL{The weights $\{W_{n}^{i}\}_{n=1\cdots N}$ determine how many copies of each trajectory are produced at time $t=t_i$. See~\cite{brewer_efficient_2018, nemoto_population-dynamics_2016} for information on the practical implementation of the algorithm.}
...
\end{equation}
\EL{IS IT TRUE?} \TL{Oui. Il faudrait un peu reécrire ce paragraphe pour que ca soit clair, voir issue \#5}
The validity of this mean-field approximation and the 
%
%
...
\EL{quid the small perturbation to separated the clones? comment required \#6}
%
...
\EL{$F_T-\overline{F_T}$ in the figure! \#4}}
\end{figure}
