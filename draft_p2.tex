...
This action can then be sampled by a Markoy chain Monte Carlo (MCMC) algorithm~\cite{duben_monte_2008,mesterhazy_anomalous_2011,mesterhazy_lattice_2013}.
...
A different approach is the Instanton method~\cite{grafke_instanton_2015}, also relying on the knowledge of an action for the dynamics.
...
The instanton approach consists in using suitable numerical methods to minimize the action and compute the instanton, as well as the associated probability~\cite{chernykh_large_2001,grafke_instanton_2013,grafke_relevance_2015,grigorio_instantons_2017}.
...
\\
Whilst Burgers turbulence provids an interesting model framework for the investigation of the properties of small scale turbulence, it is of little relevance to more realistic situations, such as industrial or geophysical flows.
...
Recently, the \cite{gktl} allowed to perorm the cumerical simulation of extreme heat waves in a simplified model of the atmosphere~\cite{ragone_computation_2018}. Such application represent a great leap in term of complexity with respect to previous application of rare event algorithms.
...
For a more detailed description of such dynamics, see the chapter 3 of ~\cite{ref:these_lestang}.
...
For more details concerning the implementation of the algorithm and the corresponding data analysis, see~\cite{these_lestang,nemoto_bouchet2016}.
...
\section{Conclusion}
\label{conlusion}
We assessedthe application of two rare event algorithms for the numerical simulation of extreme mechanical efforts on structures immersed in a turbulent flow, a situation relevant to many industrial applications.
\\
