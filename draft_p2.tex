...
\begin{figure}
  ...
  \textbf{(b)} Autocorrelation function of the drag defined as $\left( \overline{f_d(t+\tau)f_d(t)}-\overline{f_d}^2 \right) /\sigma^2$. The lag $\tau$ is normalized by the turnover-time~\eqref{eq:turnover_time}.}
  \label{fig:pdf_drag}
\end{figure}
%
...
Each experiment was run using $512$ Intel processors, each processor being assigned the computation of $N_c / 512 = 32$ clones.
The walltime for one experiment was roughly two days.
...
Nevertheless, table~\ref{tbl:nbTraj_tbl} shows that the \ac{gktl} algorithm is able to sample trajectories corresponding to extreme fluctuations of the time-averaged drag that could not be sampled through a direct sampling with equal computational cost.
