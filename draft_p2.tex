...
\title{Rare event algorithms for the simulation of extreme mechanical efforts exerted by a turbulent flow on a bluff body}
...
Turbulent flows are important in a variety of natural phenomena, industrial and civil applications with the characteristic feature to spontaneously develop intense and sporadic motions associated with extreme internal forces. In the turbulence literature, ``extreme'' can refer to fluctuations that deviate from the mean value by $O(10)$ standard deviations.
...
\\
Rare event sampling for turbulence has traditionally been approached from simplified dynamics, such as Burger's turbulence with a stochastic forcing~\cite{bec_burgers_2007}.
In this context it is possible to describe the dynamics through an action analoguous to a Gibbs measure for an equilibrium sytem.
This action can then be sampled by a Markoy chain Monte Carlo (MCMC) algorithm~\cite{duben_monte_2008,mesterhazy2011anomalous,mesterhazy2013lattice}.
Even though Monte Carlo simulations are not expected to be competitive with traditional approaches such as finite difference or pseudo-spectral methods, they provide a simulation framework in which constraints can be applied so as to certain events preferentially.
A different approach is the Instanton method~\cite{grafke2015instanton}, also relying on the knowledge of an action for the dynamics.
For stochastically driven systms, in the limit of weak noise, it can be shown that the probability of extreme events is dominated by the action minima~\cite{freidlin2013random}.
The trajectory that that minimizes the action is referred to as the \textit{instanton}~\cite{gurarie_instantons_1996}.
The instanton approach consists in using suitable numerical methods to minimize the action and compute the instanton, as well as the associated probability~\cite{grafke2015instanton,chernykh_large_2001,grafke_instanton_2013,grigorio_instantons_2017}.
Such approaches are however limited to stochastically driven dynamics, simple enough so that an action can be derived and sampled, or minimized. This is for instance the case of the stochastically driven 1D Burger's equation.
\\
Although Burgers turbulence provides an interesting model framework for the investigation of the properties of small scale turbulence, it is of little relevance to more realistic situations, such as industrial or geophysical flows.
In this paper, we discuss a thrid approach to the numerical simulation of rare events in turbuent flows, that aims at flows for which an action cannot be derived.
It is based on the application of \textit{rare event algorithms}, coupled with a numerical simulation of the flow dynamics.
Even though the original ideads can be traced back to the early 1950's, rare event algorithms have been of ever-growing interest voer the last 20 years. Successful application of such algorithms can be found in various domains such as chemistry~\cite{van_erp_elaborating_2005,escobedo_transition_2009,teo_adaptive_2016},biophysics~\cite{huber_weighted-ensemble_1996,zuckerman2017weighted,bolhuis2005kinetic}, nuclear phsics~\cite{louvin2017}, nonlinear dynamical systems~\cite{tailleur_probing_2007}, communication networks simulation~\cite{villen-altamirano_restart:_1994}.
\\
However, a large body of algorithms are limited to systems at equilibrium, for which the statioany ry measure can be sampled by means of Monte Carlo methods.
For systems far from equilibrium, such as turbulent flows, the sampling of rare events must be done at the level of the trajectories.
In the present paper with discuss two different algorithms suitable to out-of-equilibrum dynamics.
The Adaptive Multilevel Algorithm (2006)~\cite{cerou_adaptive_2007} inherits from older ideas about splitting approaches~\cite{KahnHarris1951,glasserman_look_1998,glasserman_multilevel_1999}.
In recent years, it allowed the computation of rare events in problems involving a large number of degrees of freedom, such as Nolecular Dynamics simulations~\cite{aristoff_adaptive_2015}, the computation of rare trajectoris in the Allen-Cahn equations~\cite{rolland2016computing} or rare re-laminarisation in a stochastic model of wall-turbulence~\cite{rolland_extremely_2018}.
Over the last decade, the main theoritical framework for the study of rare events in statistical physics has been the one of large deviation theory~\cite{touchette_large_2009}.
Alongside, numerical methods have been developed to sample rare events based on general ideas and algorithms~\cite{moral_feynman-kac_2004}. Among them, the \ac{gktl} algorithm~\cite{giardina_direct_2006} is particularly suited to rare events in dynamical systems~\cite{giardina_simulating_2011,Laffargue_2013}.
\\
The applicability of such algorithms to flows relevant to envorinmental or industrial applications remains open.
Recently, the \ac{gktl} algorithm allowed to perorm the cumerical simulation of extreme heat waves in a simplified model of the atmosphere~\cite{ragone_computation_2018}. Such application represent a great leap in term of complexity with respect to previous application of rare event algorithms.
In the present paper, we consider a different context of application: turbulent flows around objects.
...
\begin{figure}
  ...
  \caption{Our case study is a grid-generated turbulent flow impinging a squared obstacle located at the center of a channel (in two dimensions).  The vorticity is displayed with an arbitrary colormap from blue (negative values) to red (positive values). The Reynolds number based on the grid spacing is $Re=1200$.}
  \label{fig:illustr_ecoulement}
\end{figure}

The drag exerted by a grid-generated turbulent flow onto a squared obstacle is considered as our representative case study.
The flow is contained in a long channel as illustrated in Fig.~\ref{fig:illustr_ecoulement}.
Although real-world applications would eventually imply three-dimensional dynamics, a simplified two-dimensional setting has been chosen here to reduce the computational cost and make possible a systematic study. 
Nevertheless, this fluid-mechanical system embeds the key features that makes the application of rare event algorithms both relevant and challenging for turbulent fluid-structure interaction problems.
In particular, spatio-temporal chaos that results in the unpredictable emergence of coherent fluid structures, in turn leading to  strong fluctuations of the mechanical efforts on the structure.
In the present situation, the obstacle does not deform or move.
Through  this specific setting, our main motivation is to evaluate the operability of rare event sampling techniques to capture extreme events with a significant run-time savings.  
\\
...
Turbulent eddies are generated in the near-wake of the grid and carried downstream.
The size of the eddies that eventually interact with the square obstacle, mounted at the center of the channel, is comparable to its size, thus resulting in strong fluctuations of the drag acting on the square. 
At the outlet of the channel, an open boundary boundary is prescribed.
In addition, the flow is progressively damped within a \textit{sponge layer}~\cite{vergnault_lattice_2012} where the viscosity is artificially increased. 
\\


%% The LBM %%

The flow dynamics are computed by the~\ac{lbm} method~\cite{kruger_lattice_2017}, using a non-orthogonal central moment relaxation scheme~\ref{De_Rosis_2016}.
The \ac{lbm} solves the Navier-Stokes equations, in a weakly compressible limit, by simulating the dynamics of fictitious particles at a mesoscopic level.
The dynamics are discretized on an isotropic lattice of spacing $\Delta x$.
The \ac{lbm} is further described in appendix~\ref{sec:lbm} and references therein.

%% LB parameters  %

The lattice spacing is set to $\Delta x = R/16$, where $R$ denotes the side of the square obstacle.
The channel has a dimension $L_x \times L_y$ with $L_y = 8R$ and $L_x = 4L_y = 32R$.
The lattice therefore has a resolution of $N_y = 129$ lattice nodes in the $y$-direction and $N_x = 513$ nodes in the $x$-direction.
The grid spacing is set to $g = 8\Delta x$.
The characteristic velocity at the inlet is set to $U_0 = 0.05 \frac{\Delta x}{\Delta t}$, where $\Delta t$ is the lattice timestep.
\textcolor{red}{@todo@Emmanuel: Briefly explain why the low Mach number and the nearly incompressible limit.}
Finally, the viscosity is set so that the Reynolds number is $Re_{grid} = U_0g/\nu = 1200$.
The Reynolds number based on the mean flow velocity and the size of the obstacle is $Re_{obs} \approx 1700$.

% Boundary conditions %

The channel walls and the surface of the obstacle are decribed by the halfway bounce-back scheme~\cite{ref_BB}\textcolor{red}{@todo@Emmanuel: Provide citation for halfway bounce-back boundary condition}.
The open boundary at the outlet is based on a second order extrapolation of the velocity along the $x$-direction.
The extrapolated velocity is then enforced using a regularization based on a finite difference estimation of the local stress tensor~\cite{latt2008straight}.
The parabolic profile at the inlet is imposed by setting populations at the corresponding equilibrium, see appendix~\ref{sec:lbm}.
\\
...
Note that the ratio $\tau_0 / \tau_c$ may be viewed as a \textit{Strouhal number} $St$.
...
In this context, the occurrence of the fluctuations follows a Poisson process and~\cite{lestang_computing_2018}
\begin{equation}
...
Such plot can be obtained from a sample timeseries following a simple procedure~\cite{lestang_computing_2018}.
...
Figure~\ref{fig:return_time_instant} displays the fluctuation amplitude $a$ as a function of the return time.
According to figure~\ref{fig:return_time_instant}, the threshold amplitude $a$ is set to $7.6\sigma$ with $\sigma$ the standard deviation of the drag process.
We denote by $(t^{\star}, f_d^{\star})$ a fluctuation of peak amplitude $f_d^{\star}$ occurring at time $t^{\star}$.
...
Figure~\ref{fig:pressure_ratio} shows the relative contributions $\tilde{p}_{base}^{\star}/\tilde{f}_d^{\star}$ and $\tilde{p}_{fb}^{\star}/\tilde{f}_d^{\star}$ to the overall drag fluctuation for the base and forebody pressure fluctuations, respectively.  
\begin{figure}
  \centering
  \includegraphics[width=.8\linewidth]{pressure_ratio/pressure_ratio.png}
  \caption{\label{fig:pressure_ratio} Contribution of the forebody and base pressure fluctuation to the overall drag fluctuation for the 104 sampled extreme drag fluctuations in the control run. To each of these fluctuation correspond a unique value of the drag $f^{\star}_d$, and a unique pair ($\tilde{p}^{\star}_{base}$,$\tilde{p}^{\star}_{fb}$). The dashed lines are first order least square fits of the data. They highlight that, as the value of the fluctuation increases, so is the relative contribution of the forebody pressure fluctuation. }
\end{figure}
...
\begin{figure}
  ...
  \caption{\label{fig:density+streamlines} Pressure field and velocity streamlines in the vicinity of the obstacle at $t=t^{\star}$. Because of the blocking vortex, vorticity produced along the top boundary of the obsatcle has no other choice but to accumulate at the base of the obstacle.}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=.7\linewidth]{timeseries_extremes/timeseries_extremes.png}
  \caption{\label{fig:timeseries_extremes} Average drag timeseries in the vicinity of an extreme drag fluctuation at $t=t^{\star}$. The blue continuous line results from an average over the 104 extreme events sampled in the control run. This illustrates that extreme drag variations are very local in time, with a typical lifetime of one correlation time unit $\tau_c$.}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=.8\linewidth]{dynamics_extremes/dynamics_extremes.png}
  \caption{\label{fig:vorticity_dynamics} Vorticity dynamics associated to an extreme fluctuation of the drag acting on the square obstacle at $t=t^{\star}$.}
\end{figure}
...
To begin with, figure~\ref{fig:top_4_events_vorticity} displays the vorticity field corresponding to the peak drag value, for a subset of the extreme events sampled in the control simulation.
We observe that, in each case, extreme drag flcutuations correspond to very similar flow configurations.
More precisely, a region of strong vorticity is located in the vicinity of the base of the obstacle, where vorticity levels typically reach twice as much as the typical vorticity fluctuations displayed in figure~\ref{fig:typical_vorticity}.
...
In contrast with the typical flow dynamics, illustrated in figure~\ref{fig:typical_vorticity}, the vorticity generated within the boundary layers is not directly advectd by the means flow.
Figure~\ref{fig:density+streamlines} shows that, instead, this vorticity is constrained in the vicinity of the base of the obstacle.
...
Figure~\ref{fig:vorticity_dynamics} illustrate the time evolution of the vorticity field, in the vicinity of the drag fluctuation, for the event depicted in figure~\ref{fig:density+streamlines}.
...
As illustrated in figure~\ref{fig:timeseries_extremes}, the drag abruptly varies for $t^{\star}-\tau_c \leq t \leq t^{\star}+\tau_c$.
...
For a more detailed description of such dynamics, see chapter 3 of~\cite{lestang:tel-01974316}.
