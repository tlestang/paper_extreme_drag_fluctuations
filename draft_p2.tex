...
\begin{figure}
  \centering
    \subfloat[]{\label{fig:pdf_drag_a}\includegraphics[width=.5\linewidth]{./PDF_drag/PDF_drag.png}}
    \subfloat[]{\label{fig:pdf_drag_b}\includegraphics[width=.5\linewidth]{autocorr_drag/autocorr_drag.png}}  
...
In the following we consider the effective fluctuation $\tilde{f}_d^{\star} = f_d^{\star} - \bar{f}_d$.
It can be further decomposed into 
\begin{equation}
  \tilde{f}_d^{\star} = \tilde{p}_{fb}^{\star} - \tilde{p}_{base}^{\star},
\end{equation}
where $\tilde{p}_{fb}^{\star}$ and $\tilde{p}_{base}^{\star}$ denote the fluctuation of the forebody pressure and base pressure, respectively, that is $\tilde{p}_{base}^{\star} = p_{base}^{\star} - \bar{p}_{base}$ .
...
\begin{figure}
  ...
  \caption{\label{fig:top_4_events_vorticity} Vorticity field at $t=t^{\star}$ for some of the highest drag fluctuations in the control timeseries.
    The extreme value for the drag results from the concentration of vorticity in the vicinity of the base of the obstacle, causing a pressure drop.}
\end{figure}
...
\begin{figure}
  ...
  \caption{Pressure field and velocity streamlines in the vicinity of the obstacle at $t=t^{\star}$. Becasue of the blocking vortex, vorticity produced along the top boundary of the obsatcle has no other choice but to accumulate at the base of the obstacle.}
\end{figure}
...
% Computational cost and subsequent choice of parameters
The computational cost $C_{gktl}$ of a \ac{gktl} experiment is determined by both the duration of the trajectories $T_a$ and the number of trajectories $N$: $C_{gktl} = N_c \times T_a$.
In the following series of experiments we set $T_a = T = 10\tau_c$.
The number of the clones is set to $N=16384$.
As a consequence, the computational cost is $C_{gktl} \approx 1.6 \times 10^5 \tau_c$.
Last but not least, one must set the cloning period $\tau$.
...
\begin{equation}
  ...
  \approx \frac{1}{N}\sum_{j=1}^{N}\delta(F^{(j)}_{T}[\{\mathbf{x}\}_{0\leq t \leq T_a}] - F),
\end{equation}
computed over the set of $N=16384$ trajectories sampled by the algorithm.
Moreover, figure~\ref{fig:illustration_IS_drag} displays an estimate of the <<unbiased>> \ac{pdf} $\set{P}_0$ describing the time-averaged drag process. This estimate was computed on the basis of a control simulation of the flow spanning $T_{tot} = 10^6 \tau_c$ (see section~\\ref{sec:pdfs}).
Note that the unbiased \ac{pdf} could also be estimated using the \ac{gktl} algorithm with $k=0$.

% Limitations

For a fixed number of trajectories $N$, there is however an upper bound $k_{max}$ on the parameter $k$ above which finite
size effects harm the efficiency and accuracy of the selection procedure.
Typically, at each selection step, the majority of the clones are resampled, with the expection of only one or two.
...
This is a consequence of the fact that most of the trajectories in the biased ensemble overlap, because the resampling of
discarded trajectories is based on a very small subset of the ensemble of $N$ trajectories.
Note that in practice,there is no such sharp upper bound $k_{max}$: finite size effects become gradually relevant as the
bias $k$ is increased.
An important question regarding the application of the \ac{gktl} algorithm is therefore that of the scaling of $k_max$ as
a function of the ensemble size $N$. 
See~\cite{ref1,ref2,ref3} for further details.

For the application discussed in this paper, the order of magnitude of $k_{max}$ was assessed empirically, by assessing the
diversity of the trajectories in the biased ensemble.
...
Sampling larger fluctuations therefore requires a larger population size, and a larger computational cost.
Nevertheless, table~\ref{tbl:nbTraj_tbl} shows that the \ac{gktl} algorithm is able to sample trajectories corresponding to extreme fluctuations of the time-averaged drag that could
not be sampled through a direct sampling with equal computational cost.
For each three \ac{gktl} experiments, table~\ref{tbl:nbTraj_tbl} displays the number of trajectories that correspond to a time-average $F_{T_a} \geq a$.
Note that trajectories that overlap over more than $50\%$ of their total duration~\footnote{Trajectories that overlap over an interval $[O;t]$, with $t\geq T_a / 2$.} are counted as one.  
...
To estimate this number we assumed that the values of the time-average drag $F_T$, with $T=10\tau_c$, are Gaussian distributed.
This hypothesis is motivated by figure~\ref{fig:PDF_AVG}.
Even though such a Gaussian assumption does not hold for fluctuations $F_T \geq 4\sigma_T$,
it may still provide numbers relevant to the comparison with the algorithm.
\begin{figure}
  \centering
